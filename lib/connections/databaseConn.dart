...
class StudentDatabase {
  ...
  update(StudentDetail studentDetails) async {
    db.update('student', studentDetails.toMap(),
        where: "id=?", whereArgs: [studentDetails.id]);
  }
  ...
  searchCoincidence(var studentDetails) async {
    if(db.select('student', where: "email=?", whereArgs: [studentDetails.email])){
      return true;
    }else {
      return false;
    }
